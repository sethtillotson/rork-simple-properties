export type DefaultDocumentTemplate = { name: string; content: string };
...
{{tenant_signature}}`
  ...
  {
    name: 'Past Due Rent Notice',
    content: `PAST DUE RENT NOTICE
...
Email: {{contact_email}}`
  },
  {
    ...
    content: `MAINTENANCE REQUEST FORM
...
Date: {{response_date}}`
  ...
  {
    name: 'Notice to Enter Property',
    content: `NOTICE TO ENTER PROPERTY
...
Landlord/Agent: {{landlord_signature}} Date: {{landlord_signature_date}}`
  },
  {
    ...
    content: `SECURITY DEPOSIT RETURN LETTER
...
Email: {{contact_email}}`
  ...
  {
    name: 'Notice of Policy Change',
    content: `NOTICE OF POLICY CHANGE
